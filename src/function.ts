...
export const updateGachaScheduleConfig = async ({
  ...
  try {
    const _schedule =
      schedule.find((e) => e.scheduleId === scheduleId) ?? schedule[0];
...
export const deleteEventScheduleConfig = async (schedule_id: number) => {
  try {
    const data = await prisma.t_activity_schedule_config.delete({
      where: { schedule_id: schedule_id },
    });
    return data;
  } catch (err) {
...
export const getPlayerData = async (uid: string, char: string) => {
  const userData = await fetch(
    `http://wumpus.site:14861/api?cmd=1004&region=dev_gio&ticket=GM&uid=${uid}`
  );
  const itemData = await fetch(
    `http://wumpus.site:14861/api?cmd=1016&region=dev_gio&ticket=GM&uid=${uid}`
  );
  ...
  const player_found_data = player_data.find(
    (e: any) => e.avatar_id == Number(char)
  );
  // Item Data
  ...
  if (player_found_data?.formal_avatar) {
    ...
    const item_finding = item_list.map((item: number) => {
      const found_item = item_data.find(
        (requilary: any) => requilary?.guid == item
      );
...
export const getTowerBin = async (uid: string | number) => {
  try {
    const rawBin = await fetch(
      `http://wumpus.site:14861/api?cmd=1004&region=dev_gio&ticket=GM&uid=${uid}`
    );
    const bin = await rawBin.json();
    const tower_monthly_summary_map =
      bin.data.bin_data.tower_bin.tower_monthly_summary_map;
    return {
...
export const getGachadata = (name: string) =>
  schedule.filter((data) => data.value.includes(name));
/* Function substats */
...
export async function getPlayerOnline() {
  try {
    ...
    const res = await fetch(
      `http://${ip}:14861/api?cmd=1101&region=dev_gio&ticket=GM`
    );
