...
const command: SlashCommand = {
  ...
  execute: async (interaction: CommandInteraction) => {
    ...
    for (let i = 0; i < Object.keys(summary).length; i++) {
      ...
      const highest_dps =
        summary[name]?.monthly_combat_summary?.highest_dps_avatr_pair;
      const most_take_damage =
        summary[name]?.monthly_combat_summary?.most_take_damage_avatar_pair;
      embed.addFields({
      ...
      embed.addFields({
        name: `Nhân vật chống chịu tốt nhất: ${
          Avatar[most_take_damage?.avatar_id]
        }`,
        value: `**Damage nhận vào:** ${most_take_damage?.data.toLocaleString()}`,
      });
      const file_name = `${Avatar[highest_dps?.avatar_id].replace(
        ' ',
        '_'
      )}_Icon`;
      embed.setThumbnail(
        `https://ik.imagekit.io/asiatarget/avatar/${file_name}.png?updatedAt=1712677263090`
      );
      embeds.push(embed);
    }
