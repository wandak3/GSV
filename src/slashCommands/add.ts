  ...
  autocomplete: async (interaction) => {
    ...
    if (interaction.options.getSubcommand() === "event") {
      try {
        ...
        const filtered: { value: string; name: string }[] = eventChoices.filter(
      ...
      try {
        const focusedOption = interaction.options.getFocused(true);
        const filtered: { value: string; name: string; remark?: string }[] =
      ...
      try {
        const focusedOption = interaction.options.getFocused(true);
        const filtered: { value: string; name: string }[] =
