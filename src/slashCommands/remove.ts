import {SlashCommandBuilder, PermissionFlagsBits, CommandInteraction, ButtonStyle, ButtonBuilder, ActionRowBuilder} from 'discord.js';
		...
		try {
			...
			getGachaSchedule.map((data) => {
				...
				item_list.map((id: string | number) => {
					const find: {name: string, value: string} = item.find((item) => item.value == id) ?? {name: 'Không tìm thấy', value: 'Không tìm thấy'};
					const modifiedFound = {
						name: find.name,
			...
			const filtered: {name: string; value: string}[] = items.filter((choice: any) => choice.name.includes(focusedOption.value));
	...
	execute: async (interaction: CommandInteraction) => {
		...
		try {
			...
			if (confirmation.customId === 'confirm') {
				...
				await confirmation.update({ content: `Đã xóa sự kiện ${eventName?.name}`, components: [] });
			} else if (confirmation.customId === 'cancel') {
				await confirmation.update({ content: 'Đã hủy xóa', components: [] });
			}
		} catch (e) {
			await interaction.editReply({ content: `Không có xác nhận trong vòng 30 giây. Lệnh xóa sự kiện ${eventName?.name} đã được hủy`, components: [] });
		}
	},
};
